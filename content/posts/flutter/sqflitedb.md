...
**Note:** The `sqflite` plugin has dependency on [`path`](https://pub.dev/packages/path) plugin. Please add this if not alreay not added to your configuration file.

---

# User Interface

In this code recipe, there're only two products are used to demonstration purposes. Each of the Product has an image, title, description, and price.
...
The `positionedBlock()` widget a centered aligned `Stack`. In this `Stack`, two `Positioned` widgets are added. The first `Positioned` is added at the background to display product information in a column. The second `Positioned` widget contains the product's image.

The main screen is a `StatefulWidget` since we want to update data from local database. We add sample data to database as soon as the app starts. We'll go over database details in next section. The `initDB()` method is called rom `initState()` method. The `initState()` method executes once during screen starup-time. We want to keep data ready in database for querying in `build()` method.
...
The `_ProductListingState` widget's `build()` method is responsible for updating data at the interface. It uses `FutureBuilder` widget to render data returned from asynchronous methods like `Future`. Usually data is returned asynchronously from database or remote network operations. `FutureBuilder` makes things data rendering adaptable to asynchronous style of data rendering at user interface side.
...
The `allProducts()` method returns a list of `Product` of `Future` type. Each product entry uses `positionedBlock(...)` method to display product details. Checkout the source code for this [here]().

Let's explore the database side of things in next section.
...
Refer [`ecom_db.dart`](https://github.com/ptyagicodecamp/flutter_cookbook/blob/widgets-code/flutter_widgets/lib/persistence/ecom_db.dart).
...
The `getDatabasesPath()` provides the physical location of database on device. The `join()` methods is a utility method to join the given paths into single path as per the current platform's file seprator. You need `path` plugin to access `join()` method.
...
The `db()` will return `Database` as `Future`. Initializing database is an asynchronous operation. The database referecence is being held in `database` variable, and product table name is `products` and stored in `tableProducts` constant. A table `products` is created if doesn't exist already.
...
We want to render all products from the table to interface. In our case, there are only two products.
